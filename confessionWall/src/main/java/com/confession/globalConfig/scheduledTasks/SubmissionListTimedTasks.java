...
public class SubmissionListTimedTasks {
    ...
    private void cleanUpWallList(Integer wallId) {
        ...
        try {
            // 获取有序集合中的最后157个元素
            Set<String> last157Members = redisTemplate.opsForZSet().reverseRange(WALL_SUBMISSION_RECORD + wallId, 0, 156);
            if (last157Members.size()>=157){
                // 删除原始有序集合中的所有元素
                redisTemplate.opsForZSet().removeRange(WALL_SUBMISSION_RECORD + wallId, 0, -1);

                // 将最后157个元素重新添加到原始有序集合中
                for (String member : last157Members) {
                    redisTemplate.opsForZSet().add(WALL_SUBMISSION_RECORD + wallId, member, 0);
                }
            }
        } finally {
            lock.unlock();
