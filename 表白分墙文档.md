...
>这里就把code也通过url传值到/pages/selectSchool/selectSchool页面然后在这个页面还要拿着code这个请求往后台发送注册请求要用
...
在微信小程序端，通过调用 `wx.login` 接口可以获取到用户的临时登录凭证 `code`，但是无法直接获取到用户的 `openId`、`sessionKey` 和 `unionId`。这些敏感信息需要通过将 `code` 发送到后台服务器，再由后台服务器调用微信提供的接口 `code2Session` 来获取。
